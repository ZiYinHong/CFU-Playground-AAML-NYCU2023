...
inline void ConvPerChannel(
  ...
  if (flag==1){
    ...
    //do matrix multiple (conv)
    int32_t acc = 0;
    for (int i = 0; i < output_depth; i++){
        for (int j = 0; j < output_col; j++){
            acc = 0;
            unsigned my_start = perf_get_mcycle();
            for(int k = 0; k < kernel_output_col; k++){
                // output[i][j] += kernel_im2col_output[i][k]*im2col_output[k][j];
                acc += kernel_im2col_output[i][k]*(im2col_output[k][j]+input_offset);
            }
            unsigned my_finish = perf_get_mcycle();
            my_cycles += (my_finish - my_start);
            ...
            acc = MultiplyByQuantizedMultiplier(
                acc, output_multiplier[i], output_shift[i]);
            acc += output_offset;
            acc = std::max(acc, output_activation_min);
            acc = std::min(acc, output_activation_max);
            output_data[Offset(output_shape, 0, int(j/output_width), int(j%output_width), i)] = static_cast<int8_t>(acc);
            // output_data[j+i*(output_col)] = static_cast<int8_t>(acc);  //will have error
        }
    }
  }
}
